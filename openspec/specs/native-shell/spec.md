...
Die native Hülle bringt die Plattform als App auf iOS und Android, **ohne eine
zweite Anwendung zu werden**. Sie beschreibt, was die Hülle leisten muss, damit
genau ein Build weiterhin genau eine Wahrheit ist: derselbe Vite-Build trägt
Web und Mobil, plattform-abhängiges Verhalten hängt an einer Weiche je Belang,
und ein Fehler im Feed ist mit demselben Commit überall behoben.

Sie deckt vier Belange ab, die es im Browser nicht gibt: **wo die Sitzung
liegt** (im Gerätespeicher statt im `localStorage`, den iOS unter Druck
abräumt), **was der Erststart wiegt**, **welche Ränder geschützt sind** (Notch,
Home-Indikator, Systemleisten) und **wie eine Korrektur das Gerät erreicht,
ohne durch den Store zu gehen** — die OTA-Übernahme samt Rückweg, denn ohne
...
eine Übertragung ist bei Apple und kommt nicht zurück.
...
- **AND** dieselben Zugangsdaten authentifizieren, die schon signiert haben

#### Scenario: Ein Bündel ohne bestandenen Nachweis erreicht den Store nicht

- **WHEN** der Nachweis am Artefakt fehlschlägt — fremdes Team, Entwickler-
  statt Verteil-Zertifikat, Geräteliste im Profil, falsche Push-Umgebung oder
  eine Build-Nummer, die nicht aus dem Lauf stammt
