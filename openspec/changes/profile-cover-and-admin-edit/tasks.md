...
- [ ] 0.3 Lokalen Stack hochfahren (`supabase start`), `supabase db reset`
      **nur lokal**.
...
- [ ] 1.1 **RED**: pgTAP-Fälle in `rls_test.sql`:
...
- [ ] 1.2 Spalte `cover_url text` + `comment on column`.
- [ ] 1.3 `grant update (cover_url) on public.profiles to authenticated;` —
      **nur diese eine**, kein pauschales Grant ohne Spaltenliste (Begründung
      wörtlich aus `20260721070000_grant_update_videos.sql`).
- [ ] 1.4 `profiles_public` neu deklarieren: `cover_url` **ans Ende** der
...
- [ ] 1.5 **Golden-Snapshot nachziehen**: `supabase/tests/grants_test.sql:92`
...
- [ ] 1.6 **GREEN**: `supabase test db --local supabase/tests/rls_test.sql supabase/tests/grants_test.sql`
...
- [ ] 2.2 Tabelle anlegen: `profile_id uuid primary key references
      public.profiles(id) on delete cascade`, `paid_until date`,
      `legacy_tier text`, `legacy_price numeric(10,2)`, `legacy_source_id text`.
- [ ] 2.3 `alter table … enable row level security;` — **keine** Policy für
      `authenticated`, **kein** Grant. Zugriff nur `service_role` + die
      Admin-Funktionen.
- [ ] 2.4 Unique-Index über `nullif(btrim(legacy_source_id), '')`, partiell
      `where legacy_source_id is not null`.
- [ ] 2.5 `comment on table` + `comment on column` mit: dem Warum von
...
- [ ] 3.1 **RED**: pgTAP-Fälle für `storage.objects`, als Falltabelle gegen
...
- [ ] 3.2 Bucket `covers`: `public = true`, `file_size_limit = 2097152`
...
- [ ] 3.3 Policies `covers_insert_own` / `covers_update_own` /
      `covers_delete_own` — `is_activated()` **und** erster Pfadabschnitt
      `= auth.uid()`, wörtlich nach dem avatars-Muster.
- [ ] 3.4 **Keine** SELECT-Policy (bewusst; im Kopf begründen).
- [ ] 3.5 **GREEN** pgTAP. Zusätzlich **über die Storage-Schnittstelle** (nicht
...
- [ ] 4.4 Weißliste + **feldweises** Dekodieren nach der Tabelle in
      ...
      `raise exception`.
- [ ] 4.5 Drei Zielzeilen: `profiles` (update), `profile_contacts` (upsert),
      `profile_legacy` (upsert). **Kein `execute format(...)`** — Begründung im
      Kopf.
- [ ] 4.6 `admin_get_profile(target uuid)` — die drei Zeilen als ein `jsonb`,
...
- [ ] 4.7 `admin_find_profile(needle text)` — Suche über `auth.users.email` und
      `profiles.name`, höchstens 20 Treffer.
- [ ] 4.8 `admin_audit`-Zeile aus jeder schreibenden Funktion.
- [ ] 4.9 Rechte: `revoke execute … from public, anon;` +
      `grant execute … to authenticated;` für alle drei RPCs.
- [ ] 4.10 **GREEN**.
- [ ] 4.11 `supabase gen types typescript --local > src/lib/database.types.ts`

## 5 · Edge Function `admin-change-email`

- [ ] 5.1 **RED**: Vitest — fehlende Kennung → 401; Aufrufer ohne Admin-Rolle →
...
- [ ] 5.2 Handler nach dem Muster von `redeem-activation`. `verify_jwt = true`.
      `sub` **aus dem Gateway-geprüften JWT** lesen — nicht `getUser()`, nicht
      `getClaims()` (ES256 in Prod).
- [ ] 5.3 Admin-Prüfung mit `service_role` gegen `staff_roles`.
- [ ] 5.4 `auth.admin.updateUserById(target, { email, email_confirm: true })`,
      danach `public.revoke_sessions(target)`, danach `admin_audit`.
- [ ] 5.5 `config.toml`-Eintrag, damit CI die Function deployt.
      **Deploy wendet Functions nicht automatisch an** — der Nachweis ist ein
      Function-Deploy, nicht ein grüner Frontend-Build.
- [ ] 5.6 **GREEN** Vitest. **Zusätzlich ein Lauf gegen DEV**: gültiges,
...
- [ ] 6.1 **RED**: Vitest — mit `aspect={3}` exportiert er 1500×500; ohne Props
      weiterhin 512×512 (Regressionsschutz für den Avatar).
- [ ] 6.2 `VIEW`/`OUT` in Breite und Höhe zerlegen, Props `aspect = 1`,
      `outWidth = 512`, `label`. `rounded-full` nur bei `aspect === 1`.
- [ ] 6.3 `ProfilPage` bleibt am bestehenden Aufruf **unverändert**.
- [ ] 6.4 **GREEN**.

## 7 · Hintergrundbild hochladen (Datenschicht + Editor)

- [ ] 7.1 **RED**: Vitest für `saveProfile` — mit `coverBlob` Upload nach
      ...
      kein Objekt**.
- [ ] 7.2 `profileFormSchema` um `cover_url`; `fetchProfileEditorData` liest mit.
- [ ] 7.3 `saveProfile` lädt analog zum Avatar hoch (kein `upsert`, Zeitstempel
      im Pfad — der Upsert-Weg scheitert an der fehlenden SELECT-Policy).
- [ ] 7.4 Editor-Abschnitt: auswählen, zuschneiden (3:1), ersetzen, entfernen.
      Beim Entfernen im UI sagen, dass die Verknüpfung gelöst wird.
- [ ] 7.5 **GREEN** + **Sichtprobe lokal**, bevor committet wird.
...
- [ ] 8.1 **RED**: Vitest —
      ...
      Bezeichner)
- [ ] 8.2 Route `/admin/mitglied/:id` hinter `RequireAdmin`. **Nicht**
...
- [ ] 8.3 Suchfeld auf `/admin` über `admin_find_profile` als Einstieg.
      Zusätzlich der Bearbeiten-Button auf `/p/:id` für sichtbare Profile.
- [ ] 8.4 `ProfileEditor` bekommt einen Fremd-Modus: Ziel-ID, beide Wege über
      die RPCs, ausgeblendet werden Avatar, Cover, Interessen, Ziele, Kompass.
- [ ] 8.5 Zusätzliche Felder im Fremd-Modus: `paid_until`, die drei
      `legacy_*`-Felder, die **Kontaktadresse** und die **Login-Adresse** —
      nebeneinander, mit einem Satz dazu, was welche tut.
- [ ] 8.6 Login-Adresse ändern ruft `admin-change-email`; die Antwort
      „Sitzungen nicht beendet" wird als Hinweis gezeigt, nicht als Fehler.
- [ ] 8.7 **GREEN** + Sichtprobe lokal, mit einem **unbestätigten** Zielprofil.

## 9 · Profilansicht nach dem Mockup

- [ ] 9.1 **RED**: Vitest — Reihenfolge der Abschnitte; ein Profil ohne
...
- [ ] 9.2 `ProfileHero`: Profilbild überlappt das Hintergrundbild. Der heutige
      Kommentar sagt ausdrücklich „KEINE Überlappung" — er wird mit dem
      Mockup-Bezug **ersetzt**, nicht stehen gelassen.
- [ ] 9.3 `public-profile.ts` liest `cover_url` aus `profiles_public` mit.
- [ ] 9.4 **`dashboard.ts` ebenso** — sonst sieht ein Mitglied sein eigenes
      Hintergrundbild auf „Mein Profil" nicht, obwohl es hochgeladen hat.
      `ProfilAnsichtPage` reicht es an `ProfileHero` durch.
- [ ] 9.5 Abschnitte nach der Quellen-Tabelle im Spec-Delta: Über mich ·
      Beruf · Hobbys · Ich biete · Ich suche · Aktivitäten · Eckdaten.
- [ ] 9.6 **GREEN** + Sichtprobe in **beiden** Themes (hell und navy).

## 10 · Abschluss

- [ ] 10.1 `pnpm lint && pnpm typecheck && pnpm test && pnpm build` grün —
      Ausgabe gelesen, nicht behauptet.
- [ ] 10.2 `supabase test db --local <alle pgTAP-Dateien>` grün.
