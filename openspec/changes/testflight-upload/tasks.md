...
- [ ] 2.1 In `scripts/ios-release.workflow.test.ts` die Zusagen **schreiben,
...
- [ ] 2.2 Prüfen, dass die bestehende Zusage „prueft die Signatur am `.ipa`,
      ...
      unverändert klein.
- [ ] 2.3 Gegenprobe, dass die neuen Zusagen nicht im Leeren greifen. Je eine
...
- [ ] 3.1 `upload-artifact` **vor** die neuen Schritte ziehen. Begründung im
...
- [ ] 3.2 Validierungsschritt: `xcrun altool --validate-app` mit denselben
...
- [ ] 3.4 Kommentar über dem Schritt, der die **äußere Voraussetzung** nennt:
      ...
      gemini).
- [ ] 3.5 Den **Dateikopf berichtigen.** Er sagt heute wörtlich „Er laedt NICHT
...
- [ ] 4.1 `docs/store-assets/README.md` — die ⚠-Warnung steht im Konjunktiv
...
- [ ] 4.2 `docs/secrets.md` — die Stripe-Testrunde führt über „Wand →
...
- [ ] 4.3 `docs/technisches-handbuch.md` — die Routentabelle führt
      `/mitgliedschaft` als reguläre Route. Als umgeleitet kennzeichnen.
- [ ] 4.4 Gegenprobe, dass keine **weitere** Prosa einen der sieben entfernten
      Einstiege beschreibt: `docs/`, `src/content/`, `openspec/specs/`.

## 5. Sicherheit (Gate `cso`)

- [ ] 5.1 Belegen, dass kein `.p8` und kein Schlüsselinhalt in den Baum, ins
...
- [ ] 5.2 Prüfen, dass `altool` den Schlüssel nicht in eine Fehlermeldung
...
- [ ] 6.1 `pnpm vitest run scripts/ios-release.workflow.test.ts` grün, Zahl der
      Zusagen vorher/nachher notiert.
- [ ] 6.2 `pnpm typecheck` und `pnpm lint` grün. **Kein `pnpm format`** — nur
      `format:check`; der Baum ist mit ~396 Dateien vorbestehend unformatiert.
- [ ] 6.3 `openspec validate --all` grün.
- [ ] 6.4 Volle Suite grün, Zahl notiert.
- [ ] 6.5 **Was dieser Change NICHT belegt, ausdrücklich festhalten:** dass ein
...
- [ ] 7.1 Unabhängiger Reviewer auf dem **Diff**, mit `the-pragmatic-programmer`
