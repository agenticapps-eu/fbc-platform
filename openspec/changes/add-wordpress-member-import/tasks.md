...
- [ ] 5.2a **Widerspruch, aufgefallen beim Bauen des Bestandslesers — Entscheidung nötig, blockiert Gruppe 7.** Ein Bestandskonto **ohne** Kennung, dessen Adresse ein Quelldatensatz trägt, hat im Plan zwei entgegengesetzte Behandlungen: 4.2 sagt „Kollision — ausschliessen und den Schreiblauf abbrechen", die Anforderung „Ein Konto ohne Kennung wird wiedererkannt" sagt „erkennen, Kennung ergänzen, kein zweites Konto". In der Datenbank sehen beide Fälle **gleich** aus. Vorschlag: an `tier`/`activated_at` unterscheiden — ein vom Import angelegtes Konto ist `impact` + `activated_at is null`, eine Selbstregistrierung ist `basic`. Bis zur Entscheidung gilt die **konservative** Lesart (4.2: ausschliessen), weil die andere Richtung im Fehlerfall eine Stufe verschenkt.
