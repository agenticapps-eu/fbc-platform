...
- [ ] 5.3 Trockenlauf gegen den lokalen Stack; Zeilenzahlen in `profiles`, `profile_contacts`, `profile_legacy`, `auth.users` **und** die Objektzahl in der Ablage davor/danach gleich (gemessen, nicht behauptet)
      — **zur Hälfte belegt (15.08.), es fehlt nur die echte Quelle.** Der ganze Weg ist gegen den lokalen Stack gelaufen, mit einer künstlichen Quelldatei (3 Datensätze: angelegt, angelegt, unbrauchbare Adresse) ausserhalb des Arbeitsbaums. Gemessen davor/danach: `profiles` 2→2, `profile_contacts` 0→0, `profile_legacy` 0→0, `auth.users` 2→2, `storage.objects` 0→0. Die Konsole führte nur Datensatznummer, Kennung und Klasse; der Bericht liegt neben der Quelle mit **0600**. Offen ist der Lauf über den **echten** Export — dessen Pfad steht in keiner Notiz und ist von Donald zu erfragen.
