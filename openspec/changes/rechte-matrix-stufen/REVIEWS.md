...
| Befund | Antwort |
|---|---|
| gemini HIGH (sofortiger Welpenschutz) · opencode HIGH-3 (Proposal ≠ Design) | **Übernommen, gleiche Wurzel.** D3 neu: zweiter Schalter `platform_settings.welpenschutz_aktiv`, Vorgabe `false`. Das Ausrollen legt damit nichts um, und die Aussage des Proposals wird wahr statt gestrichen. Der Befund traf zusätzlich eine Abweichung zwischen dem, was Donald am 02.09. gewählt hat („zwei Schalter statt einem", „auf PROD ändert sich heute nichts"), und dem, was der Entwurf daraus gemacht hatte. |
