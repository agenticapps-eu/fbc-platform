...
- [ ] 1.1 Eine Migration, forward-only, mit Kopf: Befund, Entscheidung,
...
- [ ] 1.3 `create or replace function public.admin_list_members(...)` — **derselbe
      ...
      acht Zusatzspalten
- [ ] 1.4 `drop function public.admin_list_feedback()`, dann `create` mit
...
- [ ] 1.5 Der zweite Ordnungsschlüssel ist keine Kosmetik: `created_at desc`
      ...
      Migrationskopf
- [ ] 1.6 `create function public.admin_member_counts()` → `(status text, anzahl
...
- [ ] 1.7 **Rechte für alle drei neuen Funktionen ausdrücklich aussprechen.** Neue
      Funktionen tragen EXECUTE für `PUBLIC`, solange es niemand entzieht (AGE-312)
- [ ] 1.8 Migration lokal anwenden; **prüfen, dass `grants_test.sql` unberührt
      bleibt** — der Golden-Snapshot bricht an neuen Tabellen, nicht an Funktionen.
      Gemessen, nicht angenommen
- [ ] 1.9 Typen nachziehen: `src/lib/database.types.ts` für beide RPCs

## 2. Die alten Zusagen — zwei ändern, fünf ausdrücklich NICHT

- [ ] 2.1 **Die fünf argumentlosen SQL-Aufrufe in `rls_test.sql` (479, 486, 491,
...
- [ ] 2.2 Nur `rls_test.sql` Z. 500 und 504 ändern: `has_function_privilege` auf
...
- [ ] 2.3 `src/lib/feedback.test.ts:79` auf den Aufruf mit Argumenten ziehen
- [ ] 2.4 Zusage: die Vorgabewerte sind da. Ein argumentloser Aufruf liefert
      dieselbe erste Seite wie ein Aufruf mit `(25, 0)`. Das ist die Zusage, die
      2.1 überhaupt erst zu einem Wächter macht
- [ ] 2.5 Gegenprobe zu 2.2–2.4: die Vorgabewerte in einem zurückgerollten Rahmen
      entfernen und belegen, dass die fünf Aufrufe aus 2.1 brechen. Grün allein
      belegt hier nichts

## 3. Neue Zusagen an den Funktionen

- [ ] 3.1 pgTAP: Seite 2 überschneidet sich **nicht** mit Seite 1, und beide
...
- [ ] 3.2 pgTAP: `p_limit` von 0, `null` und 9999 werden geklemmt, nicht abgewiesen
- [ ] 3.3 pgTAP: die Zeile trägt `profile_id`, und sie zeigt auf **dasselbe**
      Mitglied wie `author_name` — nicht bloss „die Spalte ist da"
- [ ] 3.4 pgTAP: `admin_list_members` und `admin_member_counts` stimmen für jeden
...
- [ ] 3.5 pgTAP: ein Zustand ohne Mitglieder erscheint **mit der Zahl null**,
      nicht gar nicht
- [ ] 3.6 pgTAP: ein Mitglied und ein `matching_manager` bekommen aus
      `admin_member_counts()` einen **Fehler**, keine Zeile mit Nullen
- [ ] 3.7 pgTAP: die Rechte aller drei neuen Funktionen — `anon`, `authenticated`
      und **das fehlende PUBLIC-ACL**. Ohne die dritte Prüfung bliebe ein
      vergessenes `revoke … from public` unbemerkt
- [ ] 3.8 pgTAP: `admin_list_members` behält Signatur und Spaltensatz. Die
...
- [ ] 3.9 Gegenproben zu 3.1–3.7: jede Funktion einmal absichtlich verbiegen
...
- [ ] 4.1 `src/lib/feedback.ts`: `fetchAdminFeedback` nimmt Seite und Grösse,
      reicht `p_limit`/`p_offset` durch, gibt `profile_id` heraus. Kein Fehler
      wird zu einer leeren Liste geglättet
- [ ] 4.2 Ein Schlüssel, der die Seite trägt — sonst zeigt Seite 2 den Inhalt von
      Seite 1
- [ ] 4.3 Die Zähler-Abfrage. **Ihr Schlüssel liegt unter dem Präfix
...
- [ ] 4.4 Zusage: nach einer Zustandsänderung werden die Zähler **neu geholt**.
      Nicht das erste Rendern prüfen, sondern Mutation → Nachladen
- [ ] 4.5 `admin_list_members` selbst bleibt im Client unangetastet —
      `admin-members.test.ts:55-73` prüft ihr Args-Objekt mit `toEqual`

## 5. Die Feedback-Seite

- [ ] 5.1 Neue Seite unter `src/pages/` mit dem Inhalt von `AdminFeedbackCard`:
      Sterne, „Gefällt/Fehlt/Idee", Datum, Verfasser, Pfad. Blättern wie die
      Mitgliederliste
- [ ] 5.2 Route `/admin/feedback` hinter `RequireAdmin`, Muster `App.tsx:157-164`.
      Admin-Seiten werden hier **eager** importiert
- [ ] 5.3 Menüeintrag in `AppShell.tsx:196-210`
- [ ] 5.4 `AdminFeedbackCard` aus `AdminSettingsPage.tsx:67` entfernen; die
      Komponente auflösen, wenn sie keinen Aufrufer mehr hat
- [ ] 5.5 `AdminSettingsPage.test.tsx:61-92` von der Zusage zur **Negativ**zusage
      drehen, wie AGE-578 es mit `EinstellungenPage.test.tsx:171-178` tat. Ein
      gelöschter Test wäre keine Zusage, sondern eine Lücke
- [ ] 5.6 Zusage: ein gescheiterter Aufruf zeigt **nicht** den Leerzustand
- [ ] 5.7 Zusage auf den Menüeintrag — heute prüft **kein** Test die
      Zusammensetzung des Administrationsabschnitts
- [ ] 5.8 **Negativzusage:** ein Nicht-Admin auf `/admin/feedback` wird
      weggeleitet. Ohne sie bliebe eine Route ohne `RequireAdmin` grün
- [ ] 5.9 **Negativzusage:** `/admin/mitglied/:id` steht **nicht** im Menü. Eine
      Route mit Parameter hat dort nichts zu suchen, und ohne die Zusage fiele es
      niemandem auf

## 6. Zähler an den Reitern

- [ ] 6.1 Die Zahl an jeden der fünf Reiter (`AdminMitgliederPage.tsx:339-349`).
...
- [ ] 6.2 Die Abbildung Reiter → Zustand steht in der Fläche:
      **`mitgliedschaft → alle`**. `admin_list_members(..., 'mitgliedschaft')`
      würfe `22023` — die Kennung ist ein Darstellungsmodus, kein Zustand
- [ ] 6.3 Zusage: jeder Reiter zeigt seine Zahl; „Alle" und „Mitgliedschaft"
      dieselbe
- [ ] 6.4 Zusage: solange die Zahlen nicht da sind, steht **keine** Zahl da —
      nicht die Null (die Lehre aus AGE-582, 6.6)
- [ ] 6.5 Zusage: bei aktiver Suche bleiben die Zahlen **global**. Das ist die
...
- [ ] 7.1 `CommunityFeed` liest `?post=<id>` per `useSearchParams` und **holt den
...
- [ ] 7.2 **Der Parameter darf NICHT in `feedSeitenKey`.** Er ändert nicht, was
...
- [ ] 7.3 Der geholte Beitrag steht dem Feed voran und wird in der Liste darunter
      **herausgefiltert** — sonst steht er zweimal da und sieht wie ein
      Dublettenfehler aus
- [ ] 7.4 Nicht gefunden: eine Meldung. **Es gibt nur einen Weg dorthin** —
...
- [ ] 7.5 Zusage: mit der Kennung eines sichtbaren Beitrags steht er oben; ohne
      Parameter ändert sich nichts
- [ ] 7.6 Zusage: **ununterscheidbar.** Einmal mit der Kennung eines
...
- [ ] 7.7 Zusage: der Schlüssel der Feed-Abfrage ist mit und ohne `?post=`
      derselbe. Die scharfe Fassung von 7.2
- [ ] 7.8 Zusage: der voranstehende Beitrag erscheint **nicht** zusätzlich in der
      Liste

## 8. Die Aktivitäten-Karten — beide

- [ ] 8.1 `PublicProfilePage.tsx:240-246`: jede Zeile wird ein **`<a>`** auf
      `/aktivitaet?post=<id>`. Kein `div` mit `onClick` — das bestünde
      `fireEvent.click` und wäre trotzdem nicht bedienbar (Befund gemini)
- [ ] 8.2 **Dasselbe in `profil-widgets.tsx:242-274`** („Meine Beiträge"). Das
...
- [ ] 8.3 Ersatztext **„Beitrag ohne Text"** für den leeren Body, an beiden Karten
...
- [ ] 8.4 Zusagen je Fläche: die Zeile führt zu **ihrem** Beitrag; sie ist ein
      `<a>` mit `href`; die textlose Zeile trägt den Ersatztext

## 9. Abnahme

- [ ] 9.1 `pnpm lint && pnpm typecheck && pnpm test && pnpm build` grün — **nie**
      `pnpm format`
- [ ] 9.2 `supabase test db` mit **ausdrücklicher Dateiliste** aus `ci.yml` grün.
      Ohne die Liste meldet der Befehl FAIL, obwohl grün
- [ ] 9.3 Sichtprobe gegen den lokalen Stack, **beide Themes** — `hell` und `navy`
...
- [ ] 9.4 Sichtprobe: Deeplink von **beiden** Profilflächen, einmal auf einen
      jungen und einmal auf einen **alten** Beitrag — der alte ist der Fall, an
      dem der verworfene Entwurf gescheitert wäre
- [ ] 9.5 Sichtprobe: die fünf Zahlen stimmen mit dem überein, was der jeweilige
      Reiter dann zeigt — und sie bleiben stehen, wenn eine Suche läuft
- [ ] 9.6 Sichtprobe: eine Zustandsänderung an einem Mitglied ändert die Zahlen
      **ohne Neuladen**
- [ ] 9.7 Sichtprobe: die Feedback-Seite blättert; `/admin` trägt die Karte nicht
      mehr
