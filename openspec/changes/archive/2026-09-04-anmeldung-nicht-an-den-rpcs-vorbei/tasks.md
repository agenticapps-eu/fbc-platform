...
- [x] 2.1 **Auf PROD rein lesend zählen, ob es überbuchte Events gibt:** Events,
      ...
      **Keine Namen, keine Mailadressen ins Repo** — das Repositorium ist
      öffentlich.
- [x] 2.2 **Selbst-Check-ins sind historisch NICHT messbar** und werden deshalb
...
- [x] 2.3 Bestand der pgTAP-Läufe festhalten (Dateien und Zusagen), damit die
...
- [x] 3.1 **Weg A — Überbuchung per INSERT.** Als aktiviertes Mitglied mit
      Stufe ≥ 4 direkt einfügen, `status = 'registered'`, Event voll.
- [x] 3.2 **Weg B — Aufstieg von der Warteliste** per UPDATE.
- [x] 3.3 **Weg C — Selbst-Einchecken** (`checked_in = true`).
- [x] 3.4 **Weg D — Umhängen auf ein volles Event** (`event_id` ändern,
      `registered → registered`). Vierter Weg aus der Planungs-Review.
- [x] 3.5 Alle vier gegen den Bestand laufen lassen; jeder muss **heute
      durchgehen**. Ergebnis je mit Zahl festhalten — das belegt die Lücke,
      statt sie zu behaupten.
- [x] 3.6 **Die Falle aus dem Vorgang:** nicht das Vorhandensein der Policy
...
- [x] 4.1 Vier Zusagen, je eine pro Weg aus Gruppe 3, formuliert als „wird
      abgewiesen". Vor der Migration sind sie **rot** — das ist das RED.
- [x] 4.2 Zusage: `checked_in` scheitert am **fehlenden Spaltenrecht**, nicht an
      der Policy. Die Unterscheidung gehört in die Zusage, sonst belegt sie den
      falschen Mechanismus.
- [x] 4.3 Zusage über die effektiven Spaltenrechte per `has_column_privilege`
      für jede Spalte einzeln — nicht nur über den Golden-Snapshot (Befund
      codex, MEDIUM).

## 5. Die Migration

- [x] 5.1 `regs_write_own` von `for all` auf `for update` verengen; `using` und
      `with check` inhaltlich unverändert. Im Kopf begründen: UPDATE bleibt
      erlaubt, INSERT und DELETE nicht (Befund gemini, LOW).
- [x] 5.2 `revoke update on public.event_registrations from authenticated`,
...
- [x] 5.3 `revoke insert, delete on public.event_registrations from
      authenticated` — auf Rechte-Ebene, nicht nur über die Policy.
- [x] 5.4 Triggerfunktion, **Schicht 1**: rollenunabhängige Kapazitätsprüfung bei
      jedem Weg nach `status = 'registered'`, INSERT wie UPDATE.
- [x] 5.5 Triggerfunktion, **Schicht 2**: direkter Statuswechsel nach
...
- [x] 5.6 **`revoke execute` auf der neuen Triggerfunktion** von `public`, `anon`,
...
- [x] 5.7 `event_id`, `profile_id`, `id`, `created_at` sind nicht aktualisierbar
      — durch die Spaltenliste aus 5.2 abgedeckt, aber ausdrücklich als Zusage
      geführt, nicht als Nebenwirkung.
- [x] 5.8 `comment on policy` und `comment on function` nachziehen — in diesem
      Repo tragen die Migrationsköpfe die Entscheidungen.

- [x] 5.9 **Nachtrag 04.09., aus dem Lauf gegen die Datenbank:** die zwei
      Schichten stehen in ZWEI Triggern, nicht in einem. Als eine
      `SECURITY INVOKER`-Funktion zaehlte Schicht 1 unter der RLS des
      Schreibenden und sah null belegte Plaetze — sie war fail-OPEN, genau an
      der Zusage, die sie tragen sollte. Schicht 1 ist jetzt
      `SECURITY DEFINER`, Schicht 2 bleibt `SECURITY INVOKER`; die Reihenfolge
      haengt am Namen. Schicht 1 deckt dabei zusaetzlich Weg D ab.

## 6. GREEN

- [x] 6.1 Die vier Zusagen aus 4.1 werden grün; je das Ergebnis mit Zahl.
- [x] 6.2 Die Meldungen nennen den Grund und verwechseln die Mechanismen nicht.
...
- [x] 7.1 `register_for_event` vergibt bei freier Kapazität `registered`
      (INSERT-Zweig).
- [x] 7.2 `register_for_event` vergibt bei voller Kapazität `waitlist`.
- [x] 7.3 **`cancelled → registered` über den RPC** — Wiederanmeldung nach dem
      ...
      unbemerkt geblieben.
- [x] 7.4 **`waitlist → registered` über den RPC** bei frei gewordener Kapazität
      — ebenfalls der UPDATE-Zweig.
- [x] 7.5 `set_event_check_in` setzt als Host weiterhin `checked_in`.
- [x] 7.6 Ein Mitglied sagt weiterhin ab (`status = 'cancelled'`).
- [x] 7.7 Ein Mitglied bewertet weiterhin (`rating`) — auch an einer Zeile, die
...
- [x] 8.1 `supabase/tests/grants_test.sql` läuft rot — aus einem Tabellenrecht
      werden Spaltenrechte. **Korrektur 04.09. (Befund opencode, NIEDRIG):** der
      Zusatz „und eine Funktion kommt dazu" war falsch. Abschnitt 6 führt die
      Funktionen, die **anon ausführen darf**; die beiden neuen bekommen den
      `revoke execute` und erscheinen deshalb zu Recht NICHT. Rot wurde genau
      eine Zeile — die Tabellenrechte. Nachgesehen, nicht angenommen.
- [x] 8.2 Beim Nachziehen die **Differenz benennen**, nicht die Liste blind
      ersetzen: rot muss dieser Change sein und kein zweiter, unbeabsichtigter
      Rechteverlust.

## 9. Abnahme

- [x] 9.1 pgTAP vollständig grün, Zahl gegen den Bestand aus 2.3.
- [x] 9.2 `pnpm test` grün — der Client ist nicht angefasst, die Zahl muss stehen.
- [x] 9.3 `pnpm lint`, `pnpm typecheck`, `pnpm build` — je den **Exit-Code**.
- [x] 9.4 `openspec validate --all` grün.
- [x] 9.5 Fremdreview auf dem **Diff**, Ergebnis in `REVIEWS.md` ergänzt.
      `opencode` und `gemini`, **beide APPROVE**, vier NIEDRIG-Befunde. Zwei
      davon übernommen: Schicht 1 liest die `events`-Zeile jetzt `for update`
      (Serialisierung mit zwei Sitzungen nachgemessen), und die falsche
      Erwartung in 8.1 ist korrigiert.
