...
- [x] **RED**: pgTAP — die Tabelle existiert, RLS ist an; ein aktiviertes
...
- [x] **RED**: pgTAP — ein Admin kann `skipped_by` **nicht** auf eine fremde
      `uuid` setzen; die Policy verlangt `skipped_by = auth.uid()`.
- [x] **RED**: pgTAP — die Positivkontrolle: derselbe Admin darf anlegen,
      lesen und löschen. Ohne sie belegt jede Verneinung oben nichts.
- [x] `supabase/migrations/<ts>_release_entry_skips.sql`: `slug` als
...
- [x] Grants **ausdrücklich** aussprechen (AGE-312):
      `grant select, insert, delete … to authenticated`.
- [x] Golden-Snapshot in `supabase/tests/grants_test.sql` mitpflegen — sonst
      bricht der `migrations`-Job, auch ohne dass der Tabellenname dort steht.
- [x] Testdatei in `ci.yml` eintragen (`supabase test db` ohne Dateiliste lügt).

## 2. Datenschicht

- [x] `database.types.ts` **von Hand** um die Tabelle ergänzen —
      `supabase gen types` NIE darüberlaufen lassen.
- [x] **RED**: Test — `fetchUebersprungene` liest `release_entry_skips`;
...
- [x] **RED**: Test — `fetchAngekuendigt` fragt **ohne** `range()` ab und
      liest `body` **nicht** mit.
- [x] Die vier Funktionen in `src/lib/release-notes.ts`.

## 3. Die Rechnung: `teileAuf()`

- [x] **RED**: Die vier Zusagen von `nochNichtAngekuendigt` ziehen um und
      bleiben grün: Zugestelltes fällt raus, ein **Entwurf** versteckt nichts,
      mehrere Notes summieren sich, ohne Notes ist alles offen.
- [x] **RED**: die neuen Zusagen — Übersprungenes fällt aus `offen` und steht
...
- [x] `teileAuf()` schreiben, `nochNichtAngekuendigt()` entfernen (danach ohne
      Aufrufer).

## 4. Die Fläche

- [x] **RED**: Seitentest — ein zweites Kästchen „nicht relevant" je Zeile;
