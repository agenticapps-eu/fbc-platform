...
- [ ] 1.1 **RED**: Fälle in `supabase/tests/rls_test.sql`, alle rot: - `insert` ohne `starts_at` → abgelehnt - `ends_at <= starts_at` → abgelehnt - `ends_at is null` bei gesetztem `starts_at` → angenommen - zwei Events mit demselben `cover_path` → abgelehnt - **`cover_path` mit fremdem Präfix → abgelehnt** (Befund codex, HIGH) - `cover_path = null` → angenommen
      `plan()` mitziehen.
- [ ] 1.2 Spalten anlegen: `description text`, `ends_at timestamptz`,
      `cover_path text`, `topics text[]`.
- [ ] 1.3 `unique (cover_path)`. Begründung im Kopf: `event_cover_lesbar`
...
- [ ] 1.4 **`events_write_host` um die Pfadbindung erweitern**: im `with_check`
...
- [ ] 1.5 `alter column starts_at set not null`. DEV: 9/0, PROD: 0/0 (0.7).
- [ ] 1.6 `check (ends_at is null or ends_at > starts_at)`.
- [ ] 1.7 **Keine** Änderung an `events_visibility_check`. Die toten Werte
...
- [ ] 1.8 **GREEN**: 1.1 läuft. Zusätzlich `grants_test.sql` ausführen und die
...
- [ ] 1.9 `database.types.ts` von Hand ergänzen: die vier neuen Spalten **und**
...
- [ ] 2.1 **RED**: Fälle in `rls_test.sql`, alle rot: - aktiviertes Mitglied, sichtbares Event, nicht Host → alle
...
- [ ] 2.2 Funktion anlegen: `security definer`, `stable`,
      `set search_path = ''`, Rückgabe `table (profile_id uuid, status text)`,
      Sortierung nach `created_at`.
- [ ] 2.3 Gate, Sichtbarkeit und Opt-out **in der Funktion**:
...
- [ ] 2.4 `revoke execute … from public` und `grant execute … to authenticated`.
      **Nicht an `anon`** — ausgeloggt gibt es keine Teilnehmer, auch nicht bei
      einem öffentlichen Event.
- [ ] 2.5 `comment on function` mit zwei Sätzen: warum die Policy unberührt
      bleibt, und warum nicht öffentliche Profile fehlen.
- [ ] 2.6 **GREEN**: 2.1 läuft, inklusive des Regressionsfalls.
- [ ] 2.7 `database.types.ts` um die RPC-Signatur ergänzen.
...
- [ ] 3.2 Bucket `event-covers` anlegen: `public = false`, 2 MiB,
...
- [ ] 3.3 **Bucket-Konfiguration als eigene Assertion** (Befund codex, MEDIUM),
...
- [ ] 3.4 Funktion `public.event_cover_lesbar(objektname text)`:
...
- [ ] 3.5 `revoke execute on function … from public` und
...
- [ ] 3.6 Vier Policies: `select` für `anon, authenticated` über die Funktion;
      `insert`/`update`/`delete` nur im eigenen `{uid}/`-Präfix hinter
      `public.is_activated()` — wörtlich wie bei den drei bestehenden Buckets.
- [ ] 3.7 **Gate-Zählung für `event_cover_%`** nach dem Muster von
...
- [ ] 3.8 **GREEN**: 3.1 und 3.3 laufen.

## 4 · Migrationen anwenden

- [ ] 4.1 Lokal: `supabase db reset`, alle drei laufen sauber durch, pgTAP grün.
...
- [ ] 5.1 **RED**: Unit-Tests für `formatEventSpan`: gleicher Tag → ein Datum
      ...
      nichts.
- [ ] 5.2 `EventRow`, `EventListItem`, `EVENT_COLUMNS` um `description`,
      `ends_at`, `cover_path`, `topics` erweitern.
- [ ] 5.3 `EventInput.startsAt` von `string | null` auf **`string`** verengen —
...
- [ ] 5.4 `createEvent`/`updateEvent` um die vier Felder erweitern.
...
- [ ] 5.5 `fetchEventAttendees(eventId)`: RPC `event_attendees`, danach Namen
...
- [ ] 5.6 **Eigener Query-Key** `eventAttendeesKey(uid, eventId)` (Befund
...
- [ ] 5.7 **Invalidierung**: `eventAttendeesKey` nach Anmeldung **und**
...
- [ ] 5.8 `src/lib/event-cover.ts`: `EVENT_COVER_BUCKET`, **ein gebündelter**
...
- [ ] 5.9 Ein nicht signierbares Objekt fällt auf den Platzhalter zurück und
      reißt **nicht** die ganze Ansicht mit (Befund codex, MEDIUM). Unit-Test
      mit einer Teilantwort.
- [ ] 5.10 `selectSimilarEvents(all, event, now)`: die drei nächsten kommenden
...
- [ ] 6.1 **RED**: „leerer Termin blockiert das Absenden" und „Titel + Termin
      genügen". Der zweite ist der wichtigere: er hält fest, dass die vier
      neuen Felder optional sind.
- [ ] 6.2 Beschreibung als mehrzeiliges Feld, optional.
- [ ] 6.3 Ende als `datetime-local`, optional, neben dem Beginn. Client-seitig
      prüfen, dass es nach dem Beginn liegt — die Constraint fängt es sonst
      erst als rohen Datenbankfehler.
- [ ] 6.4 Themen: eine Zeile je Programmpunkt, optional. **Keine Anbindung an
...
- [ ] 6.5 Titelbild: `AvatarCropper` mit `aspect = 3`, Ausgabe WebP, Upload
...
- [ ] 6.6 Ein **eigener Knopf zum Entfernen** des Titelbilds; Speichern ohne
      neue Auswahl behält das bestehende Bild (5.4). Test für beide Richtungen.
- [ ] 6.7 Termin wird Pflichtfeld: `canSubmit` erweitern, Feld markieren.
- [ ] 6.8 **Vorbelegtes Bearbeiten prüfen.** `EventForm` initialisiert seinen
...
- [ ] 7.1 `CardGrid` auf `sm:grid-cols-2 lg:grid-cols-3`. Heute steht dort nur
      `sm:grid-cols-2` — der Schritt ist 2 → 3, nicht 4 → 3.
- [ ] 7.2 Kachel nach Mockup: Titelbild mit Datumsmarke (Monat über Tag) links
      oben, Typ-Marke, Titel, Von–Bis, Ort, Teilnehmerzahl, Knopf.
- [ ] 7.3 Ohne `cover_path` **und** bei nicht signierbarem Objekt ein neutraler
      Platzhalter in gleicher Höhe — sonst springt das Raster.
- [ ] 7.4 **Ein** Signieraufruf für die ganze Liste (5.8), nicht einer je
      Kachel. Test, der die Zahl der Aufrufe festhält.
- [ ] 7.5 Die Teilnehmer**zahl** kommt weiterhin aus
      ...
      **nicht** aufgerufen. Das Mockup zeigt hier eine Zahl („63 nehmen teil"),
      keine Gesichter.
- [ ] 7.6 `truncate` bei `location` beibehalten — eine `location` kann ein
      Zoom-Link sein und sprengt sonst die Spalte.
- [ ] 7.7 Erste Testdatei zu `EventCard`.

## 8 · Detailseite

Datei: `src/pages/EventDetailPage.tsx`

- [ ] 8.1 Titelbild als Header mit Datumsmarke; ohne Bild der Platzhalter.
- [ ] 8.2 Titel, Typ-Marke, Von–Bis, Ort.
- [ ] 8.3 Beschreibung; fehlt sie, entfällt der Block ganz statt leer zu stehen.
- [ ] 8.4 Themen als Häkchenliste (Mockup-Lesart, siehe `proposal.md`).
- [ ] 8.5 Veranstalter mit Bild, Name, Stufe und Verlinkung — der bestehende
...
- [ ] 8.6 Teilnehmerreihe: bis zu fünf Avatare, dann `+n`, mit Gesamtzahl aus
...
- [ ] 8.7 **Die Zahl wird nicht aus den Gesichtern gerechnet.** Weil die RPC
...
- [ ] 8.8 Anmeldeknopf, Kapazität, Warteliste, Bewertung, `HostTools`:
      **unverändert**. Regressionstests für Bewertung (`RatePanel`) und
      ...
      statt es zu lösen.
- [ ] 8.9 Ähnliche Events aus `selectSimilarEvents`, gespeist aus **derselben
...
- [ ] 8.10 Erste Testdatei zu `EventDetailPage` — mindestens: Zeitspanne,
...
- [ ] 9.1 `pnpm lint && pnpm typecheck && pnpm test && pnpm build` grün, Ausgabe
      gelesen. Vergleich gegen die Ausgangsmessung aus 0.6.
- [ ] 9.2 `supabase test db --local supabase/tests/rls_test.sql
  supabase/tests/grants_test.sql` grün, **mit** Pfadliste.
- [ ] 9.3 **Sichtprobe im Browser, lokal, vor dem Commit.** Grüne Tests haben in
...
- [ ] 9.4 **Der Nachweis, den AGE-531 ausdrücklich verlangt**, als Sonde
...
- [ ] 9.5 **Mechanik unversehrt**, am laufenden Stack durchgespielt, nicht nur
...
- [ ] 9.7 `openspec validate --all` grün.
