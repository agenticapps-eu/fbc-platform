...
- [ ] 11.5 **Das Projekt, das „PROD" heißt, erreicht niemand** (P1 in
      `REVIEW-8.7.md`). Das ausgelieferte Bundle, die Konten in `auth.users` und
      der im Browser mitgeschnittene Function-Aufruf zeigen alle drei auf
      `foelowldexkcqzewvrcf`; `viwntbodrtqxgmqyxluh` ist leer. Auch
      `VITE_SUPABASE_URL`/`ANON_KEY` in Infisical `--env=prod` zeigen dorthin —
      der Zustand, den `docs/supabase-environments.md:441` als „Rückweg"
      beschreibt. Zu entscheiden ist **nicht hier**, sondern mit 10.3/11.2: bleibt
      `viwntbodrtqxgmqyxluh` das Ziel, und wann wird umgeschaltet. Bis dahin gilt:
      jede Aussage „auf PROD gemessen" muss den Ref benennen, sonst meint sie das
      falsche Projekt.
