...
- [ ] 1.1 Delete `--accent2` **first** (definition `src/index.css:21`, overrides
...
- [ ] 1.2 Rename `--color-gold` → `--color-accent`, `--color-gold-strong` →
...
- [ ] 1.3 Fix the two `accent-color` collisions by hand — `OnboardingPage.tsx:213`
...
- [ ] 1.4 Rename the identifier `goldMix()` → `accentMix()`
      (`src/config/membershipVisuals.ts:10,27`) and update
      `membershipVisuals.test.ts`
- [ ] 1.5 Rename `--color-night` → `--color-chrome`, `--color-night-elevated` →
...
- [ ] 1.6 Rewrite every comment and label that names Gold as a colour — ~55
...
- [ ] 1.7 **Verification for this task alone**: `grep -rni "gold" src/` → 0, and
...
- [ ] 2.1 Replace the `@theme` block in `src/index.css` — blue ramp
      `--color-blue-50…950`, layer tokens at their **hell** values, semantic
      `success`/`warning`/`danger`, `--radius-card: 14px`, `--shadow-soft`
- [ ] 2.2 Add `--color-accent-ink` (text on an accent surface): `#FFFFFF` hell,
      `#081527` navy. Buttons need it; the current tree has no such token.
- [ ] 2.3 Add the single override block `html[data-variant="navy"]` — only the
      layer tokens change, per the template
- [ ] 2.4 Set `--font-display: "Fraunces", Georgia, serif`; body copy stays
      anthracite `#1E2A3A`, never pure black
- [ ] 2.5 Delete the seven `--color-fmt-*` tokens (`src/index.css:40-46`) — zero
      readers, verified
- [ ] 2.6 Re-point the tier badge colours at the blue ramp
...
- [ ] 3.1 `designVariants.ts`: `DesignVariantId` becomes `'hell' | 'navy'`,
...
- [ ] 3.6 Stop mounting `<DesignSwitcher />` in `src/App.tsx`. The component and
      its logic stay in the tree (AGE-492 §4); only the mount point goes.
- [ ] 3.7 Simplify `DesignVariantProvider` — drop `cycleVariant`, the motion preset
...
- [ ] 4.1 **tdd** Test first: `resolveInitialVariant` returns `hell` for a stored
      `sommerfest`, for `?variant=blau`, for `null`, and for garbage; returns
      `navy` only for a stored or requested `navy`
- [ ] 4.2 Reduce the inline script — `ok`/`offered` become `["hell","navy"]`,
      default `hell`, drop the `flags` map and both `dataset` writes for
      `cardStyle`/`backdrop`. Keep the `try/catch` (private mode).
- [ ] 4.3 Change the `<html data-variant="sommerfest">` attribute in `index.html`
      to `hell`, so the pre-script markup is already correct
- [ ] 4.4 Set `<meta name="theme-color">` per theme (`#FFFFFF` / `#081527`)

## 5. `member_settings.theme` (migration)

- [ ] 5.1 New migration: `alter table public.member_settings add column theme text
...
- [ ] 5.3 Confirm `supabase/tests/grants_test.sql` still passes — a new **column**
      ...
      rather than assume
- [ ] 5.4 Extend `DEFAULT_MEMBER_SETTINGS` and the `MemberSettings` type in
      `src/lib/member-settings.ts`

## 6. Theme resolution across login (pitfall 2)

- [ ] 6.1 **tdd** Test first, all four states: logged out → `localStorage` only,
...
- [ ] 6.2 Implement the resolution in `DesignVariantProvider`. The server value
...
- [ ] 6.3 On logout, do not reset the theme to `hell`; leave what the member chose.
      (`AGE-258` clears the query cache on logout — make sure this survives it.)

## 7. Theme switch in the settings

- [ ] 7.1 `ToggleRow` for hell/navy in `EinstellungenPage`, wired to the existing
      `save` mutation with its optimistic update and rollback
- [ ] 7.2 The switch takes effect immediately, not on save-confirm
- [ ] 7.3 Extend `EinstellungenPage.test.tsx`

## 8. Brand

- [ ] 8.1 New `src/components/ui/CompassMark.tsx` — four-point star in a thin ring,
      `fill="currentColor"`, path from `docs/design-system.html:252`
- [ ] 8.2 Rewrite `Logo.tsx`: drop `tone`, `DARK_CHROME_VARIANTS` and the PNG
...
- [ ] 8.3 Every `alt` and `title` becomes `eff.bee.zee`
- [ ] 8.4 Delete `CrownMark.tsx`, `public/brand/fbc-logo-crown.png`,
      `public/brand/crown-favicon.svg`
- [ ] 8.5 New `public/brand/compass-favicon.svg`, ring thickened to 2px at 16px;
      update both `<link rel>` tags in `index.html`

## 9. Self-hosted fonts

- [ ] 9.1 Remove the `@import url("https://fonts.googleapis.com/…")` at
      `src/index.css:1`
- [ ] 9.2 Fraunces (400, 500) and Inter (400, 500, 600, 700) as woff2 in
      `public/fonts/`. Cormorant Garamond is dropped — Fraunces is decided.
- [ ] 9.3 `@font-face` with `font-display: swap` and `unicode-range` for latin +
      latin-ext
- [ ] 9.4 Verify: `grep -rn "fonts.googleapis" src/ index.html` → 0, **and** no
      request to a Google host in the network tab on load

## 10. Styleguide and docs

- [ ] 10.1 Rebuild `src/pages/StyleguidePage.tsx` (dev-only, `/styleguide`) as the
...
- [ ] 10.2 Rewrite `docs/design-system.md` to the new direction, pointing at
...
- [ ] 11.1 Add a CI step to `.github/workflows/ci.yml` (`verify` job) that fails on
      `grep -rni "gold" src/` or `grep -rn "fonts.googleapis" src/ index.html`
      returning anything
- [ ] 11.2 `?variant=sommerfest`, `?variant=b`, `?variant=linkedin` fall back to
      `hell` without crashing
- [ ] 11.3 Theme switch takes effect immediately, survives reload, and survives
      logout/login
- [ ] 11.4 No flash on load in either theme — capture it, do not assert it
- [ ] 11.5 Both themes clicked through: Start · Kompass · Academy · Events ·
      Mitglieder · Aktivität · Profil · Einstellungen · Login
- [ ] 11.6 WCAG AA contrast in both themes, tier badges and tags included
- [ ] 11.7 `pnpm lint && pnpm typecheck && pnpm test && pnpm build` green
