...
function SidebarContent({
  ...
  collapsed?: boolean;
  /**
   * Reicht bis zum `FeedbackButton` durch (AGE-688). Nur die Schublade setzt
   * ihn: sie trägt selbst `aria-modal` und muss es abgeben, solange das
   * portalisierte Formular über ihr liegt.
   */
  ...
  // Support (AGE-904 für die Einträge, AGE-929 für die Form): ein GEWÖHNLICHER
  // Abschnitt, zwischen „Mein Bereich" und „Administration". Bis AGE-929 war er
  // ein Sonderbau am Fuss der Leiste — nicht klappbar, andere Trennlinie,
  // andere Abstände, und er scrollte nicht mit.
  //
  // Wie „Administration" KEIN navItem-Abschnitt: seine Einträge stehen hier,
  // nicht in `navItems`. `/hilfe/tutorials` bleibt dort `section: "sub"`, und
  // damit bleibt die Zusage „genau sieben sichtbare Menüeinträge"
  // (`nav.test.ts`) unberührt. Hilfe ist nachgeordnet, kein achter Menüpunkt.
  //
  // ANDERS als „Administration" an keiner Bedingung: Hilfe ist weder eine Frage
  // der Stufe noch eine der Rolle (Spec `support-tutorials`). Ausgeloggt bleibt
  // davon die Überschrift mit „Tutorials" — `FeedbackButton` gibt ohne Konto
  // selbst `null` zurück. Genau so stand es vor AGE-929 auch schon da.
  //
  // „Tutorials" schliesst über `onNavigate` die Schublade. Der Feedback-Eintrag
  // darf das GERADE NICHT: sein Formular hängt an ihm, und die Schublade zu
  // schliessen nähme es mit — deshalb ist er ein `nachtrag` und kein Eintrag.
  sections.push({
    title: "Support",
    klappbar: true,
    items: [{ path: "/hilfe/tutorials", label: "Tutorials" }],
    nachtrag: <FeedbackButton collapsed={collapsed} onOffenChange={onFeedbackOffenChange} />,
  });
    ...
    >
      ...
      {mobileNavOpen && (
        ...
        >
          ...
          >
            {/* Der Support-Abschnitt steht seit AGE-929 IN `SidebarContent`,
                zwischen „Mein Bereich" und „Administration" — auch hier. Die
                Schublade reicht nur noch `onFeedbackOffenChange` durch: sie
                trägt selbst `aria-modal` und gibt es ab, solange das
                portalisierte Formular über ihr liegt (AGE-688). */}
            <SidebarContent
